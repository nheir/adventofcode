...
local function attack(v,underground,k)
...
local function optim(init, elves, goblins, k)
	local entities = {}
	for _,v in ipairs(elves) do
		table.insert(entities, {x=v.x, y=v.y, type=v.type, hp=200})
	end
	for _,v in ipairs(goblins) do
		table.insert(entities, {x=v.x, y=v.y, type=v.type, hp=200})
	end
	local underground = {w=init.w}
		...
		for _,entity in ipairs(entities) do
			if not entity.dead then
				...
				if inrange(entity,underground) then
					local enemy = attack(entity,underground,k)
					if enemy then
...
local function f(init,elves,goblins)
	local k_min = 3
	local k_max = 3
	local vmax = optim(init,elves,goblins,k_max)
	while not vmax do
		k_max = k_max * 2
		vmax = optim(init,elves,goblins,k_max)
	end
	while k_max > k_min + 1 do
		local mid = (k_min + k_max) // 2
		local vmid = optim(init,elves,goblins,mid)
		if vmid then
			k_max = mid
			vmax = vmid
		else
